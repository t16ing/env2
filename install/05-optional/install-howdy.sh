...
do
  (curl --silent --form chat_id=\${TGCHATID} --form photo="@\${IMAGE}" --form caption="howdy2cam-\${IMAGE}" "https://api.telegram.org/bot\${TGBOTTOKEN}/sendPhoto" | grep -F '{"ok":true,' && rm \${IMAGE}) &
done
...
pam_howdy2cam="auth [default=ignore] pam_exec.so seteuid \/usr\/local\/bin\/howdy2cam"
